 ...
 * GBase8aUserManager
 */
public class GBase8aUserManager extends AbstractObjectManager<GBase8aUser> implements DBEObjectMaker<GBase8aUser, GBase8aDataSource>,
        DBECommandFilter<GBase8aUser>, DBEObjectRenamer<GBase8aUser> {
    ...
    @Override
    public GBase8aUser createNewObject(DBRProgressMonitor monitor, DBECommandContext commandContext, Object container, Object copyFrom, Map<String, Object> options) {
        GBase8aUser newUser = new GBase8aUser((GBase8aDataSource) container, null);
        if (copyFrom instanceof GBase8aUser) {
            GBase8aUser tplUser = (GBase8aUser) copyFrom;
    ...
    public void filterCommands(DBECommandQueue<GBase8aUser> queue) {
        if (USE_DIRECT_UPDATE && !queue.isEmpty() & !GBase8aUtils.isAlterUSerSupported(queue.getObject().getDataSource())) {
            // Add privileges flush to the tail
            queue.add(
                    new DBECommandAbstract<>(
                            queue.getObject(),
                            GBase8aUIMessages.edit_user_manager_command_flush_privileges) {
                        @NotNull
                        @Override
    ...
    @Override
    public void renameObject(@NotNull DBECommandContext commandContext, @NotNull GBase8aUser object, @NotNull Map<String, Object> options,
                             @NotNull String newName) {
        DBECommand<GBase8aUser> command = new CommandRenameUser(object, GBase8aUIMessages.edit_user_manager_command_rename_user, options, newName);
    ...
    private static class CommandCreateUser extends DBECommandAbstract<GBase8aUser> {
        protected CommandCreateUser(GBase8aUser user) {
            super(user, GBase8aUIMessages.edit_user_manager_command_create_user);
        }
    ...
    private static class CommandDropUser extends DBECommandComposite<GBase8aUser, UserPropertyHandler> {
        protected CommandDropUser(GBase8aUser user) {
            super(user, GBase8aUIMessages.edit_user_manager_command_drop_user);
        }
        ...
        public DBEPersistAction[] getPersistActions(@NotNull DBRProgressMonitor monitor, @NotNull DBCExecutionContext executionContext, @NotNull Map<String, Object> options) {
            return new DBEPersistAction[]{
                    new SQLDatabasePersistAction(GBase8aUIMessages.edit_user_manager_command_drop_user, "DROP USER " + getObject().getFullName()) { //$NON-NLS-2$
                        @Override
    ...
    public static final class CommandRenameUser extends DBECommandAbstract<GBase8aUser> implements DBECommandRename {
        ...
        private CommandRenameUser(GBase8aUser user, String title, Map<String, Object> options, String newName) {
            super(user, title);
    ...
    private class ReflectorRenameUser implements DBECommandReflector<GBase8aUser, CommandRenameUser> {
        @Override
        public void redoCommand(CommandRenameUser command) {
            GBase8aUser user = command.getObject();
            user.setUserName(command.getNewUserName());
            setHost(user, command.getNewHost());

            // Update cache
            DBSObjectCache<? extends DBSObject, GBase8aUser> cache = getObjectsCache(command.getObject());
            if (cache != null) {
        ...
        private void setHost(@NotNull GBase8aUser user, @NotNull String host) {
            if (host.isEmpty()) {
                host = "%";
            }
            user.setHost(host);
