...
public class CloseResourcePlanHandler extends AbstractHandler {
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        if (selection instanceof IStructuredSelection) {
            ...
            if (element instanceof DBNDatabaseItem) {
                ((DBNDatabaseNode) element).getName();
                ...
                try (JDBCSession dbsession = DBUtils.openMetaSession(dataSource.getMonitor(), dataSource, "close resource plan");
                     JDBCStatement stmt = dbsession.createStatement()) {
                    if (!vcName.equalsIgnoreCase("default")) {
                        stmt.execute("deactive resource plan on vc " + vcName);
                    } else {
                        stmt.execute("set global active_resource_plan=''");
                    }
                    MessageBox box = new MessageBox(UIUtils.getActiveWorkbenchShell());
                    box.setMessage(GBase8aMessages.close_resourceplan_success);
                    box.open();
                } catch (Exception e) {
