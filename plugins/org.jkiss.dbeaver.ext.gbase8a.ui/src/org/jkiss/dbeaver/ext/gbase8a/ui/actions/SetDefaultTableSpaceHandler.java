...
public class SetDefaultTableSpaceHandler extends AbstractHandler {
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        if (selection instanceof IStructuredSelection) {
            ...
            if (element instanceof DBNDatabaseItem) {
                ...
                String sql = "use ";
                try (JDBCSession dbsession = DBUtils.openMetaSession(dataSource.getMonitor(), dataSource, "set default tablespace");
                     JDBCStatement stmt = dbsession.createStatement()) {
                    if (dataSource.isVCCluster()) {
                        stmt.execute("use vc " + vcName);
                    }
                    sql = sql + "`" + dbName + "`.`" + gbase8aTableSpace.getName() + "` as default tablespace";
                    stmt.execute(sql);
                    UIUtils.showMessageBox(null, GBase8aMessages.message_information_title, GBase8aMessages.dialog_setdefault_tablespace_ok, 2);
                } catch (Exception e) {
