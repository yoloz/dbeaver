...
public class ShowResourcePoolUsageOnCoordinatorsHandler extends AbstractHandler {
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        if (selection instanceof IStructuredSelection) {
            ...
            if (dataSource.isVCCluster() && !"Default".equalsIgnoreCase(vcName)) {
                vcName = resourcePool.getVcName();
            }
            String name = resourcePool.getName();

            GBase8aShowResourcePoolUsageOnNodesDialog dialog = new GBase8aShowResourcePoolUsageOnNodesDialog(UIUtils.getActiveWorkbenchShell(), dataSource, true);

            List<GBase8aResourcePoolUsage> list_usage = new ArrayList<>();
            String createSql = "show resource pool ";
            if (dataSource.isVCCluster() && !"Default".equalsIgnoreCase(vcName)) {
                createSql = createSql + "usage on coordinators where resource_pool_name=? and vc_name=?";
            } else {
                createSql = createSql + "usage on coordinators where resource_pool_name=?";
            }
