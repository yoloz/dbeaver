...
import org.jkiss.dbeaver.ext.gbase8a.ui.views.GBase8aShowResourcePoolStatusOnNodesDialog;
...
public class ShowResourcePoolStatusOnCoordinatorsHandler extends AbstractHandler {
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        if (selection instanceof IStructuredSelection) {
            ...
            GBase8aShowResourcePoolStatusOnNodesDialog dialog = new GBase8aShowResourcePoolStatusOnNodesDialog(UIUtils.getActiveWorkbenchShell(), dataSource, true);
            ...
            if (dataSource.isVCCluster() && !"Default".equalsIgnoreCase(vcName)) {
                createSql = createSql + "status on coordinators where resource_pool_name=? and vc_name=?";
            } else {
                createSql = createSql + "status on coordinators where resource_pool_name=?";
            }
            try (JDBCSession dbsession = DBUtils.openMetaSession(dataSource.getMonitor(), dataSource, "show resource pool usage on coordinators");
                ...
                try (JDBCResultSet result = stmt.executeQuery()) {
                    while (result.next()) {
                        ...
                        usage.setServied_tasks(result.getString("servied_tasks"));
                        usage.setWaiting_avg_time(result.getString("waiting_avg_time"));
                        usage.setRunning_avg_time(result.getString("running_avg_time"));
                        usage.setSample_time(result.getString("sample_time"));
