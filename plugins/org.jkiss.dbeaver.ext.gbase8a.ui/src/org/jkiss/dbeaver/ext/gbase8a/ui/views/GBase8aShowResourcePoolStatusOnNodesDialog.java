...
public class GBase8aShowResourcePoolStatusOnNodesDialog extends BaseDialog {
    ...
    public GBase8aShowResourcePoolStatusOnNodesDialog(Shell parentShell, GBase8aDataSource dataSource, boolean isCoordinator) {
        super(parentShell, GBase8aMessages.dialog_show_resource_pool_status_on_nodes_title, null);
        this.dataSource = dataSource;
        this.isCoordinator = isCoordinator;
        if (isCoordinator) {
            setTitle(GBase8aMessages.dialog_show_resource_pool_status_on_coordinators_title);
        }
    }
    ...
    protected Composite createDialogArea(Composite parent) {
        ...
        TableColumn tbServied_tasks = new TableColumn(this.table, 0);
        tbServied_tasks.setWidth(100);
        tbServied_tasks.setText(GBase8aMessages.dialog_show_resource_pool_status_servied_tasks);

        TableColumn tbWaiting_avg_time = new TableColumn(this.table, 0);
        tbWaiting_avg_time.setWidth(150);
        tbWaiting_avg_time.setText(GBase8aMessages.dialog_show_resource_pool_status_waiting_avg_time);

        TableColumn tbRunning_avg_time = new TableColumn(this.table, 0);
        tbRunning_avg_time.setWidth(150);
        tbRunning_avg_time.setText(GBase8aMessages.dialog_show_resource_pool_status_running_avg_time);

        TableColumn tbSample_time = new TableColumn(this.table, 0);
        tbSample_time.setWidth(150);
        tbSample_time.setText(GBase8aMessages.dialog_show_resource_pool_status_sample_time);

        if (this.isCoordinator) {
            tbRunning_tasks.dispose();
            tbWaiting_tasks.dispose();
            tbCpu_usage.dispose();
            tbMem_usage.dispose();
    ...
    class TableViewerLabelProvider implements ITableLabelProvider {
        public String getColumnText(Object element, int columnIndex) {
            ...
            if (GBase8aShowResourcePoolStatusOnNodesDialog.this.isCoordinator) {
                if (columnIndex == 4) {
                    return usage.getServied_tasks();
                }
                if (columnIndex == 5) {
                    return usage.getWaiting_avg_time();
                }
                if (columnIndex == 6) {
                    return usage.getRunning_avg_time();
                }
                if (columnIndex == 7) {
                    return usage.getSample_time();
                }
            } else {
                if (columnIndex == 4) {
