...
public class GBase8aForeignKeyConfigurator implements DBEObjectConfigurator<GBase8aTableForeignKey> {
    ...
    @Override
    public GBase8aTableForeignKey configureObject(@NotNull DBRProgressMonitor monitor, @Nullable DBECommandContext commandContext,
                                                  @Nullable Object table, @NotNull GBase8aTableForeignKey foreignKey, @NotNull Map<String, Object> options) {
        return UITask.run(() -> {
            EditForeignKeyPage editPage = new EditForeignKeyPage(
                GBase8aUIMessages.edit_foreign_key_manager_title,
                foreignKey,
