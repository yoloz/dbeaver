...
public class GBase8aCommandChangeUser extends DBECommandComposite<GBase8aUser, UserPropertyHandler> {
    ...
    protected GBase8aCommandChangeUser(GBase8aUser user) {
        super(user, GBase8aUIMessages.edit_command_change_user_name);
    }
    ...
    public DBEPersistAction[] getPersistActions(@NotNull DBRProgressMonitor monitor, @NotNull DBCExecutionContext executionContext, @NotNull Map<String, Object> options) {
        ...
        if (newUser) {
            ...
            actions.add(
                    new SQLDatabasePersistAction(GBase8aUIMessages.edit_command_change_user_action_create_new_user, script.toString()) {
                        @Override
        ...
        final GBase8aDataSource dataSource = getObject().getDataSource();
        if (GBase8aUtils.isAlterUSerSupported(dataSource)) {
            StringBuilder script = new StringBuilder();
            if (generateAlterScript(script, includeUserPassword)) {
                actions.add(new SQLDatabasePersistAction(GBase8aUIMessages.edit_command_change_user_action_update_user_record, script.toString()));
            }
        } else {
            String updateSQL = generateUpdateScript();
            if (updateSQL != null) {
                actions.add(new SQLDatabasePersistAction(GBase8aUIMessages.edit_command_change_user_action_update_user_record, updateSQL));
            }
    ...
    private boolean generateCreateScript(@NotNull StringBuilder script) {
        final GBase8aUser object = getObject();
