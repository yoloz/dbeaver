...
import org.jkiss.dbeaver.ext.gbase8a.ui.views.GBase8aCreateConsumerGroupDialog;
...
public class CreateConsumerGroupHandler extends AbstractHandler {
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        if (selection instanceof IStructuredSelection) {
            ...
            GBase8aCreateConsumerGroupDialog dialog = new GBase8aCreateConsumerGroupDialog(UIUtils.getActiveWorkbenchShell(), dataSource, vcName);
            if (dialog.open() != 0) {
                return null;
            }
            String groupName = dialog.getName();
            if (!groupName.isEmpty()) {
                try (JDBCSession dbsession = DBUtils.openMetaSession(dataSource.getMonitor(), dataSource, "create consumer group");
                     JDBCStatement stmt = dbsession.createStatement()) {
                    if (dataSource.isVCCluster()) {
                        stmt.execute("use vc " + vcName);
                    }
                    String createSql = "create consumer group ";
                    if (dataSource.isVCCluster()) {
                        createSql = createSql + vcName + ".";
                    }
                    createSql = createSql + groupName;
