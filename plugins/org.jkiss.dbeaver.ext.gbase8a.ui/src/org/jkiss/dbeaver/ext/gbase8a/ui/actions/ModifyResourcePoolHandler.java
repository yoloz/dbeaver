...
public class ModifyResourcePoolHandler extends AbstractHandler {
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        if (selection instanceof IStructuredSelection) {
            ...
            GBase8aCreateResourcePoolDialog dialog = new GBase8aCreateResourcePoolDialog(UIUtils.getActiveWorkbenchShell(), dataSource, resourcePool, false);
            if (dialog.open() != 0) {
                return null;
            }
            String name = dialog.getName();
            if (!name.isEmpty()) {
                try (JDBCSession dbsession = DBUtils.openMetaSession(dataSource.getMonitor(), dataSource, "modify resource pool");
                    ...
                    if (dataSource.isVCCluster() && !"Default".equalsIgnoreCase(vcName)) {
                        ...
                        createSql = createSql + vcName + "`.`" + name + "` set (";
                    } else {
                        createSql = createSql + name + "` set (";
                    }
                    ...
                    String type = resourcePool.getType();
