...
public class RefreshTableSpaceHandler extends AbstractHandler {
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        if (selection instanceof IStructuredSelection) {
            ...
            if (element instanceof DBNDatabaseItem) {
                ...
                String sql = "refresh tablespace ";
                try (JDBCSession dbsession = DBUtils.openMetaSession(dataSource.getMonitor(), dataSource, "refresh tablespace");
                     JDBCStatement stmt = dbsession.createStatement()) {
                    if (dataSource.isVCCluster()) {
                        stmt.execute("use vc " + vcName);
                    }
                    sql = sql + "`" + dbName + "`.`" + gbase8aTableSpace.getName() + "` storage usage";
                    stmt.execute(sql);
                    UIUtils.showMessageBox(null, GBase8aMessages.message_information_title, GBase8aMessages.dialog_refresh_tablespace_ok, 2);
                } catch (Exception e) {
