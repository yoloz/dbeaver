...
public class GBase8aIndexConfigurator implements DBEObjectConfigurator<GBase8aTableIndex> {
    ...
    public GBase8aTableIndex configureObject(@NotNull DBRProgressMonitor monitor, @Nullable DBECommandContext commandContext,
                                             @Nullable Object parent, @NotNull GBase8aTableIndex index, @NotNull Map<String, Object> options) {
        return UITask.run(() -> {
            GBase8aEditIndexPage editPage = new GBase8aEditIndexPage(index);
            if (!editPage.edit()) {
                return null;
            }
            ...
            for (DBSEntityAttribute tableColumn : editPage.getSelectedAttributes()) {
                ...
                Integer length = (Integer) editPage.getAttributeProperty(tableColumn, GBase8aEditIndexPage.PROP_LENGTH);
                index.addColumn(
                        new GBase8aTableIndexColumn(
                                index,
    ...
    private static class GBase8aEditIndexPage extends EditIndexPage {

        public static final String PROP_LENGTH = "length";

        private int lengthColumnIndex;

        GBase8aEditIndexPage(GBase8aTableIndex index) {
            super(GBase8aUIMessages.edit_index_manager_title, index, index.getDataSource().supportedIndexTypes());
        }
