...
public class GBase8aFunction extends AbstractProcedure<GBase8aDataSource, GBase8aCatalog> implements GBase8aSourceObject, DBPRefreshableObject {
    private DBSProcedureType procedureType;
    private transient String clientBody;
    private String charset;

    public GBase8aFunction(GBase8aCatalog catalog) {
        super(catalog, false);
        this.procedureType = DBSProcedureType.FUNCTION;
    }


    public GBase8aFunction(GBase8aCatalog catalog, ResultSet dbResult) {
        super(catalog, true);
        loadInfo(dbResult);
    }
    ...
    private void loadInfo(ResultSet dbResult) {
        setName(JDBCUtils.safeGetString(dbResult, "ROUTINE_NAME").trim());
        setDescription(JDBCUtils.safeGetString(dbResult, "ROUTINE_COMMENT").trim());
        String procType = JDBCUtils.safeGetString(dbResult, "ROUTINE_TYPE");
        this.procedureType = (procType == null) ? DBSProcedureType.FUNCTION : DBSProcedureType.valueOf(procType.toUpperCase(Locale.ENGLISH));


        this.charset = JDBCUtils.safeGetString(dbResult, "CHARACTER_SET_CLIENT").trim();

        this.description = JDBCUtils.safeGetString(dbResult, "ROUTINE_COMMENT").trim();
    }
            ...
            if (!this.persisted) {
                ...
                try (JDBCSession session = DBUtils.openMetaSession(monitor, getDataSource(), "Read procedure declaration");
                    ...
                    if (dbResult.next()) {
                        ...
                        this.clientBody = GBase8aUtils.dealFunctionSqlDefiner(this.clientBody);


                        this.clientBody = GBase8aUtils.dealFunctionSqlGualifiedName(getFullyQualifiedName(DBPEvaluationContext.DDL), this.clientBody);
                    } else {
                        this.clientBody = "";
                    }
    ...
    public Collection<GBase8aProcedureParameter> getParameters(DBRProgressMonitor monitor) throws DBException {
        return getContainer().functionsCache.getChildren(monitor, getContainer(), this);
    }
    ...
    public DBSObject refreshObject(@NotNull DBRProgressMonitor monitor) throws DBException {
        return getContainer().functionsCache.refreshObject(monitor, getContainer(), this);
    }
