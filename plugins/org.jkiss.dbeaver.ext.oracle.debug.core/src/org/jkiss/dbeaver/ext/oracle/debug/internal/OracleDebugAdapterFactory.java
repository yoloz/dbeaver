...
import org.jkiss.dbeaver.ext.oracle.debug.internal.impl.OracleDebugControllerFactory;
import org.jkiss.dbeaver.model.DBPDataSourceContainer;

public class OracleDebugAdapterFactory implements IAdapterFactory {
    ...
    public <T> T getAdapter(Object adaptableObject, Class<T> adapterType) {
        if (adapterType == DBGControllerFactory.class) {
            if (adaptableObject instanceof DBPDataSourceContainer) {
                ...
                if (ds.getDriver().getDataSourceProvider() instanceof OracleDataSourceProvider) {
                    return adapterType.cast(new OracleDebugControllerFactory());
                }
            }
        } else if (adapterType == DBGResolver.class) {
            if (adaptableObject instanceof DBPDataSourceContainer) {
                DBPDataSourceContainer ds = (DBPDataSourceContainer) adaptableObject;
                if (ds.getDriver().getProviderId().equals("oracle")) {
                    return adapterType.cast(new OracleResolver(ds));
                }
            }
        }
        return null;
